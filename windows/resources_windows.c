...
void resources_load_texture(const char *filename, int filtering, texture_t *tex)
{
	FILE *hndl = malloc(sizeof(FILE));
	fopen_s(&hndl, filename, "rb");
...
void resources_load_font(const char* filename, font_t* font)
{
	...
	if (hndl != NULL)
	{
		...
		fontFn[0] = '\0';
		sprintf_s(fontTexFilename, 256, "%s.raw", filename);
